...
parser.add_argument('--optimizer', default='SGLD', type=str, help='optimizer', choices=['sgd', 'adam', 'amsgrad', 'rmsprop', 'TUSLA', 'SGLD','SGHMC'])
...
def get_ckpt_name(dataset='f_mnist', seed=111, model='xxx', optimizer='sgd', lr=0.1, momentum=0.9,
    ...
    name = {
        ...
        'SGHMC': 'seed{}-lr{}-gamma{}-beta{:.1e}-wdecay{}'.format(seed, lr, 0.5, beta, weight_decay),
    }[optimizer]
...
if args.model == 'slfn':
    ...
    net = TLFN(input_size=input_size, hidden_size=100, output_size=output_size)

    net.hidden_layer2.weight.requires_grad = False
    net.output_layer.weight.data = torch.full([output_size, 100], 82.60)
    net.output_layer.weight.requires_grad = False
...
optimizer = { 'sgd': optim.SGD(net.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay),
              ...
              'SGHMC': SGHMC(net.parameters(), lr=args.lr, gamma=0.5, beta=args.beta, weight_decay=args.weight_decay)
}[args.optimizer]
